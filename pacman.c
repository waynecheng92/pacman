...
int totalGenerated = 0;                   //The total number of node being generated
int totalExpanded = 0;                    //The total nunber of node being explored
int maxDepth = 0;                         //The maximal depth of node being reached
float start, end;                         //The start and the end time of the game
...
void ExitProgram(const char *message) {
    ...
    // Print all the needed data to output.txt
    if (propagation == max) {
    ...
    fprintf(fp, "Expanded/Second = %f\n", totalExpanded / totalTime);
    fprintf(fp, "Score = %d\n", Points);
...
void MainLoop() {
    ...
    do {
        ...
        if(ai_run){
             ...
             */
            move_t selected_move = get_next_move( current_state, budget, propagation, ai_stats, &totalGenerated, &totalExpanded, &maxDepth);
